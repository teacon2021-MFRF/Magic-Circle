...
public class ResearchTableContainer extends ContainerBase {
    ...
    public ResearchTableContainer(int id, PlayerInventory playerInventory, BlockPos pos, World world, PlayerKnowledges knowledge) {
        ...
        this.itemResearchContain = world.getRecipeManager().getAllRecipesFor(JsonConfigs.Type.ITEM_RESEARCH_JSONCONFIG_TYPE).stream().filter(jsonConfigItemResearch -> jsonConfigItemResearch.getIngredient().test(research_table.inventory.getItem(TileResearchTable.Slot.ITEM_TO_ANALYSIS.ordinal()))).findFirst();
        this.testPaperResearchContain = Optional.ofNullable(ItemTestPaper.getTest(research_table.inventory.getItem(TileResearchTable.Slot.TEST_PAPER.ordinal())));
